 ...
 */
export function parseTranscript(raw: string, agent: string, model?: string): Transcript {
  if (!raw?.trim()) {
    return {
      agent,
      model,
 ...
 */
export function parseTranscriptSummary(raw: string, agent: string): TranscriptSummary {
 ...
 * @returns Transcript
 */
export function loadTranscript(content: string, agent?: string, model?: string): Transcript {
