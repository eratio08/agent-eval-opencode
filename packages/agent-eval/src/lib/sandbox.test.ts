...
import { collectLocalFiles, IGNORED_PATTERNS, type SandboxFile, splitTestFiles, TEST_FILE_PATTERNS } from './sandbox.js'
...
describe('sandbox utilities', () => {
  ...
  describe('splitTestFiles', () => {
    it('separates test files from workspace files', () => {
      ...
      expect(workspaceFiles.map((f) => f.path).sort()).toEqual(['package.json', 'src/App.tsx'])
      expect(testFiles.map((f) => f.path).sort()).toEqual(['EVAL.tsx', 'PROMPT.md'])
    })
  })
